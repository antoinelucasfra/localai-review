...
function Pandoc(el)
  ...
  -- Local patch: replace words-* shortcode placeholder spans with real counts.
  -- (Shortcodes expand before filters run, so they can't read counts themselves.)
  local counts = {
    body = body_words,
    ref = ref_words,
    append = appendix_words,
    note = note_words,
  ...
  el.blocks = pandoc.walk_block(pandoc.Div(el.blocks), {
    Span = function(sp)
      ...
      local sum, n = 0, 0
      for key in spec:gmatch("[^+]+") do
        if counts[key] ~= nil then sum = sum + counts[key]; n = n + 1 end
      end
