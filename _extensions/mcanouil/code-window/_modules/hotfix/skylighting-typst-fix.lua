...
local function process_typst_inline(el)
  ...
  -- The snippet is written through a separate Pandoc pass, which may emit
  -- Skylighting token macros (#NormalTok(...), #StringTok(...), ...). Those
  -- macros are only defined document-wide when the main document contains at
  -- least one highlighted code block, so documents whose code all flows
  -- through code-windows/includes would fail with "unknown variable". Rewrite
  -- token calls to self-contained raw(...) so snippets never depend on them.
  rendered = rendered:gsub('#[%w]+Tok%(', '#raw(')
